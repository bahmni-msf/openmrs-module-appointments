...
public class PatientDoubleBookingConflict implements AppointmentConflictType {
    ...
    private boolean isAppointmentOverlapping(Appointment patientAppointment, Appointment appointment) {
        ...
        return isTimeBetween(appointment.getStartDateTime(), startTime, endTime)
                || isTimeBetween(appointment.getEndDateTime(), startTime, endTime);
    }

    private boolean isTimeBetween(Date date, Date start, Date end) {
        return isAfter(date, start) && isBefore(date, end);
    }

    private boolean isAfter(Date date, Date ref) {
        return !date.before(ref);
    }

    private boolean isBefore(Date date, Date ref) {
